...
When you select a database, additional files are generated (e.g. `prisma/schema.prisma` + `prisma.config.ts` for Prisma, or `src/db/` for Drizzle/TypeORM). When Docker is enabled, you get a `Dockerfile`, `.dockerignore`, and `docker-compose.yml`. See the [Database](/docs/features/database) and [Docker](/docs/features/docker) pages for details.
